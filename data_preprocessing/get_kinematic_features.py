...
def get_velocity(raw_df, bodyparts, frame_rate):
    ...
    for bodypart in bodyparts:
        ...
        for trial_number, group in df.groupby('trial'):
            ...
            for i in range(1, len(group)):
                vx = group[disp_x_col].iloc[i] / (1 / frame_rate)
                vy = group[disp_y_col].iloc[i] / (1 / frame_rate)
    ...
    for bodypart in bodyparts:
        ...
        for trial_number, group in df.groupby('trial'):
            ...
            for i in range(2, len(group)):
                ax = (group[vel_x_col].iloc[i] - group[vel_x_col].iloc[i - 1]) / (1 / frame_rate)
                ay = (group[vel_y_col].iloc[i] - group[vel_y_col].iloc[i - 1]) / (1 / frame_rate)
